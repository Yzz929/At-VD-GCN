...
def anorm(p1,p2,p11,p21): #p1[0] x  p1[1] y
    NORM = math.sqrt((p1[0]-p2[0])**2+ (p1[1]-p2[1])**2)
    NORM1 = (((p11[0]-p1[0])*(p2[0]-p1[0]))+((p11[1]-p1[1])*(p2[1]-p1[1])))*(((p21[0]-p2[0])*(p2[0]-p1[0]))+((p21[1]-p2[1])*(p2[1]-p1[1])))
    if NORM1 < 0 or NORM == 0:
        return 0
    return 1/(NORM)
...
def seq_to_graph(seq_,seq_rel,norm_lap_matr = True):
    ...
    #             l2_norm = anorm(step_rel[h],step_rel[k])#V[s,h,h],V[s,h,h+1]
    #             A[s,h,k] = l2_norm
    #             A[s,k,h] = l2_norm
    V = np.zeros((seq_len, max_nodes, 2))
    A = np.zeros((seq_len, max_nodes, max_nodes))
...
class TrajectoryDataset(Dataset):
    """Dataloder for the Trajectory datasets轨迹数据集的编码器"""
    def __init__(
        ...
        - skip: Number of frames to skip while making the dataset制作数据集时要跳过的帧数
        - threshold: Minimum error to be considered for non linear traj
        when using a linear predictor
        - min_ped: Minimum number of pedestrians that should be in a seqeunce
        - delim: Delimiter in the dataset files数据集文件中的分隔符
        """
